...
export default class AdminAuthController {
  private formatAdminUserResponse(adminUser: any): AdminAuthResponse {
    return {
      success: true,
  ...
  public async signup(req: any, res: any, next: any) {
    try {
      ...
      return res.status(200).json(this.formatAdminUserResponse(createdAdminUser));
    } catch (error) {
      console.log(error);
  ...
  public async login(req: any, res: any, next: any) {
    try {
      ...
      return res.status(200).json(this.formatAdminUserResponse(foundAdminUser));
    } catch (error) {
      console.log(error);
